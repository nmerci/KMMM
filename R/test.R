...
#tested
test_khizanov_maiboroda <- function(n, m)
{
  data <- sample_data(n, m, 1:n)
  
  khizanov_maiboroda(data$censored_data, data$mixture_weights)
}

#tested
test_ryzhov <- function(n, m, clusters)
{
  data <- sample_data(n, m, clusters)
  
  ryzhov(data$censored_data, replicate_rows(data$mixture_weights, get_frequencies(clusters)), clusters)
}
...
plot_estimate <- function(x, y)
{
  ...
  for(i in 1:ncol(y))
  {
    ...
    lines(x=x, y=pchisq(x, i), type="l", col="blue")
  }
}
